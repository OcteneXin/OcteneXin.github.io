...
与大家的直觉相反的是，随机分布反而是自然界中最少见的分布型。接下来请允许我搬运一点点生物知识过来~~我保证，立刻，马上讲代码！~~
...
完全遵循集群分布的典型特性有：南瓜、西瓜、甜浆果丛、甘蔗、花卉、草丛、蘑菇、仙人掌、睡莲。在源码内，这些ConfiguredFeature都是Feature类的RANDOM_PATCH通过某种config而来的。

但奇怪的是，树的生成不太遵循集群分布：从源码看来，树在区块内完全是随机放置的。但由于空间检查、位置冲突等原因，大多数树最终表现为均匀分布，尤其是在较茂密的生物群系中。
